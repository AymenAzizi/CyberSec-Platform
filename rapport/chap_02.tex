...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.90\textwidth,height=0.32\textheight,keepaspectratio]{img/fig_2_2_asm.png}
...
\subsection{Comparatif des Solutions Existantes d'Audit et d'ASM}
\par Pour positionner rigoureusement notre travail, nous avons confront\'e les capacit\'es des outils d'audit individuels et des plateformes du march\'e \`a notre solution int\'egr\'ee. Le tableau~\ref{tab:existing_solutions} synth\'etise cette \'etude comparative.
...
\hline
G\'en\'eration de Rem\'ediation IA & Non & Non & Non & Recommandations fixes & Non & \textbf{Oui (LLM Local Contraint)} \\
\hline
...
\hline
Confidentialit\'e des Donn\'ees & Publique & Locale & Locale & H\'eberg\'ee (Cloud tiers) & Locale & \textbf{100\,\% Locale} \\
\hline
\end{tabular}}%
\end{table}

\section{DevSecOps et S\'ecurit\'e de la Cha\^ine Logicielle}
\subsection{Distinction Fondamentale : S\'ecurit\'e DU Syst\`eme vs S\'ecurit\'e PAR le Syst\`eme}
\par Une clarification m\'ethodologique essentielle s'impose concernant le r\^ole du DevSecOps dans notre projet :
\begin{itemize}
    \item \textbf{La S\'ecurit\'e DU Syst\`eme (Cha\^ine DevSecOps CI/CD) :} Elle s'applique au code source de la plateforme elle-m\^eme. D\'eclench\'ee automatiquement \`a chaque \texttt{git push} ou \texttt{pull request} sur GitHub Actions, elle v\'erifie que le code applicatif, les d\'ependances et les conteneurs Docker de notre solution sont exempts de vuln\'erabilit\'es avant tout d\'eploiement.
    \item \textbf{La S\'ecurit\'e PAR le Syst\`eme (Moteur de Scans ASM) :} Elle constitue la fonctionnalit\'e m\'etier op\'erationnelle offerte aux analystes. D\'eclench\'ee \`a la demande depuis l'interface web, elle ex\'ecute les outils de reconnaissance (Nmap, Nuclei, Gobuster, OSINT) pour d\'etecter les failles pr\'esentes sur les sites web et infrastructures des clients audit\'es.
\end{itemize}
...
\subsection{Comparatif des Moteurs d'Inf\'erence d'IA Locale}
...
\begin{table}[H]
\centering
\caption{Analyse Comparative des Moteurs d'Inf\'erence d'IA Locale}
\label{tab:comp_ai}
\resizebox{\textwidth}{!}{%
\begin{tabular}{|L|L|L|L|L|}
\hline
...
\end{tabular}}%
\end{table}
\par \textbf{Justification du Choix d'Ollama :} Ollama s'int\`egre parfaitement dans notre stack Docker Compose en mode conteneuris\'e sans interface graphique, exposant une API REST standardis\'ee (\texttt{http://ollama:11434}) et g\'erant automatiquement les quantifications de mod\`eles (GGUF Q4\_K\_M).

\subsection{Comparatif et Justification du Mod\`ele de Langage (LLM)}
\par Afin de justifier scientifiquement le choix du mod\`ele de langage embarqu\'e pour la synth\`ese des constats et la g\'en\'eration de code de rem\'ediation (\textit{Remediation-as-Code}), nous avons r\'ealis\'e une \'etude comparative entre les mod\`eles open-source de pointe et les APIs Cloud propri\'etaires (tableau~\ref{tab:comp_llm_models}).

\begin{table}[H]
\centering
\caption{Analyse Comparative des Mod\`eles de Langage (LLM) pour la Rem\'ediation Cybers\'ecurit\'e}
\label{tab:comp_llm_models}
\resizebox{\textwidth}{!}{%
\begin{tabular}{|L|C|C|C|C|C|C|}
\hline
\textbf{Crit\`ere d'\'Evaluation} & \textbf{Qwen2.5-Coder:7B} & \textbf{CodeLlama:7B} & \textbf{DeepSeek-Coder:6.7B} & \textbf{LLaMA-3.1:8B} & \textbf{Mistral-7B-Inst} & \textbf{APIs Cloud (GPT-4 / Claude)} \\
\hline
Sp\'ecialisation Code Syst\`eme & \textbf{Tr\`es \'Elev\'ee (Bash/Ansible)} & Mod\'er\'ee & \'Elev\'ee & G\'en\'eraliste & G\'en\'eraliste & \'Elev\'ee \\
\hline
Confidentialit\'e \& Souverainet\'e & \textbf{100\,\% Locale (Z\'ero fuite)} & 100\,\% Locale & 100\,\% Locale & 100\,\% Locale & 100\,\% Locale & \color{red}\textbf{Non (Exfiltration Cloud US)} \\
\hline
Conformit\'e RGPD / NDA Audit & \textbf{Totale} & Totale & Totale & Totale & Totale & \color{red}\textbf{Violation de confidentialit\'e} \\
\hline
Empreinte M\'emoire (Quantifi\'e) & \textbf{4,7 Go (Q4\_K\_M)} & 4,8 Go & 4,5 Go & 5,2 Go & 4,9 Go & N/A (Consommation Cloud) \\
\hline
Ex\'ecution CPU / GPU Modeste & \textbf{Oui ($<$15s par script)} & Oui & Oui & Plus lente & Plus lente & N/A \\
\hline
Respect Sch\'emas JSON Stricts & \textbf{Excellent (Pydantic)} & Faible (erreurs) & Bon & Bon & Mod\'er\'e & Excellent \\
\hline
Score HumanEval (Benchmark) & \textbf{85,4\,\%} & 67,8\,\% & 79,2\,\% & 72,6\,\% & 60,4\,\% & 90,2\,\% \\
\hline
\rowcolor{green!15} \textbf{Retenu} & \textbf{Oui} & Non & Non & Non & Non & Non \\
\hline
\end{tabular}}%
\end{table}

\par \textbf{Justification Fondamentale du Choix de Qwen2.5-Coder:7B :}
\begin{enumerate}
    \item \textbf{Imp\'eratif de Confidentialit\'e et Souverainet\'e des Donn\'ees :} Dans un contexte d'audit de cybers\'ecurit\'e, transmettre les adresses IP internes, les versions logicielles et les preuves de vuln\'erabilit\'es \`a des APIs Cloud externes (comme OpenAI ou Anthropic) constitue une \textbf{violation majeure du secret professionnel, du RGPD et des clauses de confidentialit\'e (NDA)}. Le mod\`ele \texttt{qwen2.5-coder} s'ex\'ecute \`a 100\,\% en local sans aucune \'emission de trafic sortant.
    \item \textbf{Excellence sur le Code d'Administration et Durcissement :} Contrairement aux mod\`eles g\'en\'eralistes (LLaMA-3.1, Mistral) qui souffrent d'hallucinations sur la syntaxe Bash ou Ansible, \texttt{qwen2.5-coder} a \'et\'e pr\'e-entra\^in\'e sp\'ecifiquement sur des d\'ep\^ots de code source et de scripts d'infrastructure, obtenant un score de \textbf{85,4\,\%} sur le benchmark HumanEval.
    \item \textbf{Adh\'erence Rigide aux Formats JSON Contraints :} Le mod\`ele g\'en\`ere de fa\c{c}on d\'eterministe des r\'eponses conformes aux sch\'emas Pydantic valid\'es par notre microservice IA, avec indication pr\'ecise des num\'eros de lignes de journaux bruts (\textit{citations}) pour \'eliminer tout risque d'hallucination.
    \item \textbf{L\'eg\`eret\'e Op\'erationnelle :} Gr\^ace \`a la quantification GGUF \texttt{Q4\_K\_M}, le mod\`ele occupe seulement \textbf{4,7 Go de m\'emoire vive}, permettant son inf\'erence rapide m\^eme sur des serveurs standards ou en environnement CPU/GPU d\'edi\'e sans surco\^ut mat\'eriel.
\end{enumerate}
...
\section{Patrons de Conception Appliqu\'es (\textit{Design Patterns})}
\par L'architecture de la plateforme CyberSec-ASM mobilise plusieurs patrons de conception (\textit{Design Patterns}) reconnus pour r\'epondre aux imp\'eratifs de modularit\'e, d'extensibilit\'e, d'isolation et de maintenabilit\'e.

\subsection{Patrons de Cr\'eation}
\begin{itemize}
    \item \textbf{Patron Fabrique (\textit{Factory Method}) :} Utilis\'e dans les microservices Python pour instancier dynamiquement l'adaptateur de scanner appropri\'e (\texttt{NmapService}, \texttt{NucleiService}, \texttt{GobusterService}, \texttt{WPScanService}) \`a partir du profil et des modules d'audit demand\'es, sans coupler le contr\^oleur \`a l'impl\'ementation concr\`ete.
    \item \textbf{Patron Singleton :} Appliqu\'e aux gestionnaires de connexions vers la base de donn\'ees PostgreSQL/Apache AGE et vers le serveur d'inf\'erence Ollama. Ce mod\`ele garantit qu'une unique instance du pool de connexions et du client LLM est partag\'ee au sein de chaque processus de microservice, optimisant ainsi l'utilisation des ressources m\'emoire.
\end{itemize}

\subsection{Patrons Structurels}
\begin{itemize}
    \item \textbf{Patron Fa\c{c}ade (\textit{Facade}) :} Impl\'ement\'e \`a travers le service back-end \texttt{MicroserviceClient} et l'orchestrateur \texttt{ScanOrchestrator}. Ces composants exposent une interface unifi\'ee simplifi\'ee aux contr\^oleurs Laravel, en masquant la complexit\'e sous-jacente des requ\^etes r\'eseau asynchrones, de la gestion des jetons d'authentification inter-services, du m\'ecanisme de r\'eessais (\textit{exponential backoff}) et de la s\'erialisation JSON.
    \item \textbf{Patron Adaptateur (\textit{Adapter}) :} Mat\'erialis\'e par la classe abstraite \texttt{BaseScannerService}. Ce patron transforme les interfaces h\'et\'erog\`enes des outils CLI natifs (arguments de ligne de commande, variables d'environnement, flux de sortie texte) en une interface standardis\'ee conforme aux attentes du pipeline de traitement.
\end{itemize}

\subsection{Patrons Comportementaux}
\begin{itemize}
    \item \textbf{Patron Observateur / Publication-Souscription (\textit{Pub/Sub}) :} Orchestr\'e par le moteur d'\'ev\'enements Redis Streams. Les microservices d'audit publient des messages d'avancement (\textit{progress events}) sans conna\^itre les consommateurs, tandis que les workers d'arri\`ere-plan et les canaux de notification IHM captent ces \'ev\'enements de mani\`ere totalement d\'ecoupl\'ee.
    \item \textbf{Patron Patron de M\'ethode (\textit{Template Method}) :} Structurant le pipeline de normalisation \texttt{ResultParser}. Il d\'efinit le squelette invariant de l'algorithme d'ingestion en six phases successives (d\'ecompression, validation syntaxique, extraction d'entit\'es, mapping CVE/CWE, calcul de s\'ev\'erit\'e, citation des logs), en d\'el\'eguant aux sous-classes le parsing sp\'ecifique \`a chaque scanner.
    \item \textbf{Patron \'Etat (\textit{State}) :} Mod\'elisant le cycle de vie formel d'un scan d'audit (\texttt{queued} $\rightarrow$ \texttt{running} $\rightarrow$ \texttt{completed} / \texttt{failed} / \texttt{cancelled}), assurant que les transitions d'\'etat respectent rigoureusement les r\`egles de gestion et interdisent les actions incoh\'erentes.
\end{itemize}

\subsection{Patron d'Architecture}
\begin{itemize}
    \item \textbf{Patron D\'ep\^ot (\textit{Repository / Data Access Object}) :} Utilis\'e dans le socle Laravel 11 avec l'ORM Eloquent pour isoler formellement les r\`egles m\'etier de la couche de persistance relationnelle, facilitant ainsi les tests unitaires et la maintenance \'evolutive.
\end{itemize}

\section*{Conclusion}
\par Ce chapitre a \'etabli l'\'etat de l'art scientifique et conceptuel de la cybers\'ecurit\'e, de l'ASM, des scanners d'audit, du DevSecOps, de l'IA locale, des microservices et des patrons de conception appliqu\'es. Les \'etudes comparatives syst\'ematiques ont justifi\'e l'ensemble des choix technologiques retenus. Le chapitre suivant d\'etaillera l'analyse approfondie des besoins, la mod\'elisation formelle des menaces STRIDE et la conception architecturale de la plateforme.
