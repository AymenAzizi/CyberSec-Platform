...
\par Pour une plateforme d'audit o\`u les t\^aches de balayage (scans Nmap, Gobuster) peuvent durer de plusieurs secondes \`a plusieurs dizaines de minutes, une architecture purement synchrone HTTP conduirait in\'evitablement \`a des blocages de requ\^etes (\textit{timeouts}). L'adoption d'un mod\`ele asynchrone pilot\'e par \'ev\'enements via \textbf{Redis Streams} permet de d\'ecoupler le portail web de l'ex\'ecution des scanners : les requ\^etes d'audit sont ins\'er\'ees dans un flux persistant, puis consomm\'ees par des processus d\'edi\'es (\textit{workers}) capables de g\'erer les r\'eessais automatiques avec repli exponentiel.
...
\hline
Microservices d'Audit & Flask 3.1 (Python 3.12) & L\'eg\`erement conteneuris\'e, contr\^ole absolu sur les sous-processus CLI. \\
\hline
