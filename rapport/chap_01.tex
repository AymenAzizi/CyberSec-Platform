...
\begin{itemize}
    \item \textbf{Ing\'enierie Logicielle \& Web Avanc\'e :} Conception et r\'ealisation d'architectures applicatives complexes sous des frameworks modernes (Laravel, Node.js, Python), int\'egrant des exigences \'elev\'ees de mont\'ee en charge et d'ergonomie utilisateur.
...
\par La figure~\ref{fig:organigramme_designet} illustre la structure organisationnelle de Designet Web Agency. L'agence est dirig\'ee par une Direction G\'en\'erale chapeautant la Direction Technique (CTO) et la Direction Op\'erationnelle.
...
\begin{itemize}
    \item Un portail web applicatif d\'evelopp\'e sous \textbf{Laravel 11} assurant l'authentification s\'ecuris\'ee, le contr\^ole d'acc\`es RBAC granulaire, la gestion des projets et le rendu ergonomique.
    \item Cinq microservices sp\'ecialis\'es en \textbf{Python Flask} (reconnaissance multi-outils, s\'ecurit\'e offensive / sandbox, renseignement passif OSINT, moteur IA contraint, passerelle d'API).
    \item Un moteur d'ex\'ecution asynchrone pilot\'e par \textbf{Redis Streams} garantissant la r\'esilience, la tol\'erance aux pannes et le passage \`a l'\'echelle.
    \item Une base de donn\'ees unifi\'ee sous \textbf{PostgreSQL 16} avec l'extension \textbf{Apache AGE} permettant la persistance relationnelle et le requ\^etage openCypher du graphe de connaissances.
    \item Un moteur d'inf\'erence locale (\textbf{Ollama}) h\'ebergeant le mod\`ele \texttt{qwen2.5-coder} pour la synth\`ese contextuelle et la production de code de rem\'ediation (\textit{Remediation-as-Code}).
    \item Une cha\^ine \textbf{DevSecOps} automatis\'ee sous GitHub Actions assurant la s\'ecurit\'e intrins\`eque du code source et des conteneurs de production.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.90\textwidth]{img/fig_1_1_scrum.png}
    \caption{Vue d'Ensemble du Processus Agile Scrum Appliqu\'e au Projet}
