...
def proxy(path: str = "") -> Any:
    ...
    downstream_name, target_url = _resolve_downstream(full_path)
    if not target_url:
        return jsonify({"error": f"no route for {request.path}"}), 404

    # Build the upstream URL.
    upstream = target_url.rstrip("/") + "/" + path.lstrip("/")
...
def _resolve_downstream(full_path: str) -> Tuple[str | None, str | None]:
