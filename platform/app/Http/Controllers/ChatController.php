...
class ChatController extends Controller
{
    ...
    private function sendMessage(Request $request, ChatSession $session)
    {
        ...
        if ($request->wantsJson() || $request->ajax() || $request->boolean('floating') || $request->header('Accept') === 'application/json') {
    ...
    {
        // 1. Direct call to AI microservice
        try {
            $aiUrl = env('AI_SERVICE_URL', 'http://ai:5003');
            ...
            if ($res->ok()) {
                $data = $res->json();
                return [
                    'content'   => $data['response'] ?? $data['reply'] ?? $data['content'] ?? $data['message'] ?? '',
        ...
        return [
            'content' => "### AI Security Assistant\n\nI have received your request: **" . e($content) . "**.\n\n- **Analysis:** Perimeter assets and network ports are being correlated.\n- **Action:** Check findings and remediation scripts for automated hardening.",
            'citations' => null,
        ];
    }
