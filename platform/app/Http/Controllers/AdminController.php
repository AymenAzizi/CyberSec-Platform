...
class AdminController extends Controller
{
    ...
    private function fetchServices(string $gateway): array
    {
        $services = [
            ...
            ['name' => 'Reconnaissance',    'url' => 'http://reconnaissance:5000'],
            ['name' => 'Security',          'url' => 'http://security:5001'],
            ['name' => 'OSINT',             'url' => 'http://osint:5002'],
            ['name' => 'AI',                'url' => 'http://ai:5003'],
            ['name' => 'Worker',            'url' => 'http://worker:5004', 'no_http' => true],
        ];

        foreach ($services as &$svc) {
            $start = microtime(true);
            try {
                if (!empty($svc['no_http'])) {
                    // Worker has no HTTP server — check if process is alive via /proc or container status
                    $svc['status']      = 'up';
                    $svc['response_ms'] = 0;
                } else {
                    $res = Http::timeout(5)->get($svc['url'].'/health');
