...
class SecurityController extends Controller
{
    ...
    public function sandbox()
    {
        ...
        $containers = collect();
        $endpoints = [
            env('SECURITY_SERVICE_URL', 'http://security:5001') . '/sandbox/test',
            config('services.api_gateway.url', 'http://api-gateway:8080') . '/api/security/sandbox/test',
        ];

        foreach ($endpoints as $url) {
            try {
                $res = Http::timeout(3)->post($url, ['action' => 'status']);
                if ($res->ok()) {
                    $running = $res->json('running', []);
                    $containers = collect($running);
                    break;
                }
            } catch (\Throwable $e) {
                // Try next
        ...
        // Fallback: read local sandboxes from /tmp/cybersec_sandboxes.json
    ...
    public function launchSandbox(Request $request)
    {
        if ($request->isMethod('GET')) {
            return redirect()->route('security.sandbox');
        }
        ...
        $payload = [
            'action'     => 'start',
            'target_app' => strtolower($validated['app']),
            'app'        => strtolower($validated['app']),
            'image'      => $validated['image'],
        ];

        // Try the Security microservice first
        try {
            $secUrl = env('SECURITY_SERVICE_URL', 'http://security:5001');
            $res = Http::timeout(10)->post($secUrl.'/sandbox/test', $payload);
            if ($res->ok()) {
                $data = $res->json();
                if (empty($data['error'])) {
                    return back()->with('success', "Sandbox {$validated['app']} launched in isolated container.");
                }
            }
        } catch (\Throwable $e) {
            // Fall through
        ...
        try {
            ...
            $res = Http::timeout(10)->post($gateway.'/api/security/sandbox/test', $payload);
            if ($res->ok()) {
                $data = $res->json();
                if (empty($data['error'])) {
        ...
        // Fallback: register a local simulated vulnerable application container
        try {
            $container = $this->launchLocalSandbox($validated['app'], $validated['image']);
            return back()->with('success', "Sandbox {$validated['app']} launched on port {$container['port']} (sandboxed simulation).");
        } catch (\Throwable $e) {
            return back()->with('error', 'Sandbox launch: '.$e->getMessage());
        }
    }

    /**
     * Launch a local simulated vulnerable app (DVWA, SQLi-Labs, etc.).
     * Used when external Docker daemon is unreachable.
     */
