...
class OsintController extends Controller
{
    ...
    public function run(Target $target)
    {
        ...
        // Try the OSINT microservice directly first, then via gateway.
        $targetValue = $target->domain_url ?: $target->ip_address ?: 'example.com';
        // Strip scheme if present for domain validation
        $domainOnly = preg_replace('#^https?://#i', '', rtrim($targetValue, '/'));
        
        $endpoints = [
            env('OSINT_SERVICE_URL', 'http://osint:5002') . '/passive',
            config('services.api_gateway.url', 'http://api-gateway:8080') . '/api/osint/passive',
        ];
