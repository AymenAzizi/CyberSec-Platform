...
<body>
    ...
    <main class="px-6 py-6">
        ...
        @php
            $allScripts = $report->scan ? $report->scan->findings->flatMap->remediationScripts : collect();
        @endphp
        @if ($allScripts->isNotEmpty())
        <section class="pdf-card">
            <h2>Remediation-as-Code (Automated Hardening Scripts)</h2>
            <div style="font-size: 11px; color: #64748b; margin-bottom: 12px;">
                The following hardened scripts were generated by the local AI engine (<code>qwen2.5-coder</code>) for automated fix deployment:
            </div>
            @foreach ($allScripts as $script)
                <div style="background: #0f172a; color: #e2e8f0; border-radius: 8px; padding: 12px; margin-bottom: 12px; font-family: monospace; font-size: 11px;">
                    <div style="color: #38bdf8; font-weight: bold; margin-bottom: 6px; border-bottom: 1px solid #334155; padding-bottom: 4px; display: flex; justify-content: space-between;">
                        <span>📜 {{ $script->title ?: 'Hardening Script' }} ({{ strtoupper($script->language) }})</span>
                        <span style="color: #94a3b8; font-size: 10px;">Finding: {{ $script->finding?->title ?? 'N/A' }}</span>
                    </div>
                    @if ($script->explanation)
                        <div style="color: #94a3b8; font-size: 10px; margin-bottom: 6px; font-family: sans-serif;">{{ $script->explanation }}</div>
                    @endif
                    <pre style="margin: 0; white-space: pre-wrap; color: #a5f3fc; line-height: 1.4;"><code>{{ $script->code }}</code></pre>
                </div>
            @endforeach
        </section>
        @endif
