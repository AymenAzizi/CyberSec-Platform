...
<script>
    ...
    window.targetsByProject = @json($targetsByProject);
    ...
    function renderTargets() {
        const pid = projectSelect ? String(projectSelect.value) : '';
        ...
        const list = window.targetsByProject[pid] || [];
        list.forEach((t) => {
        ...
        if (list.length >= 1) {
            ...
            const opt = document.createElement('option');
            opt.value = '';
            opt.textContent = 'Default Target (scanme.nmap.org)';
            targetSelect.appendChild(opt);
        }
    }
    ...
    if (projectSelect) {
        ...
        projectSelect.addEventListener('input', renderTargets);
    }
    if (targetSelect) {
    ...
    function syncAggressive() {
        ...
        const confirmBox = document.getElementById('aggressive-confirm');
    ...
    if (projectSelect && projectSelect.value) renderTargets();
</script>
</div>
@endsection
